  ...
  const InputChange = (event)=>{
    ...
    const Modifyproduct = (event)=>{
        ...
        if (Formconfirmation(Modify) === 6) {
          ...
          axios.post(`http://localhost/MY_PROJECTS/electronic_project2/product.php?update=${id}`,Data).then((res)=>{ 
...
return (
  ...
  <>
      <form className='w-[90%] flex gap-5 flex-col items-center relative left-1/2 -translate-x-1/2 mt-10 pb-4' onSubmit={Modifyproduct}>
          <h1 className='text-yellow-800 font-bold text-[20px] md:text-[30px] text-center'>Modifier Product</h1>
          ...
          <button type='submit' className='w-[120px] h-9 rounded-md sh bg-white text-yellow-800 duration-500 transition-all hover:text-white hover:bg-yellow-800 self-start'>Modify</button>
          </div>
      </form>
  </>
)
